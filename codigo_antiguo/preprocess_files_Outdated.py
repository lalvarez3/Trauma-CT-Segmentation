...
# task_name = 'Task505_SpleenTrauma' #change here for different task name
task_name = 'Task504_LiverTrauma' #change here for different task name
...
def make_z_last(img):
    ...
    if x != y:
        ...
        img_array = np.permute(img_array, (2,1,0))
        img_array = sitk.GetImageFromArray(img_array)
        img_array = img_array.CopyInformation(img)
        return img_array
